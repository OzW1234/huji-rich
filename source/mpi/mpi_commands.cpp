...
vector<vector<vector<size_t> > > MPI_exchange_data(const Tessellation3D& tess, vector<vector<vector<size_t> > > const& cells)
{
	...
	for (size_t i = 0; i < correspondents.size(); ++i)
	{
		...
		if (size == 0)
		{
			MPI_Isend(&temp, 1, my_MPI_SIZE_T, correspondents[i], 4, MPI_COMM_WORLD, &req[2 * i]);
			MPI_Isend(&send_sizes[i][0], 1, my_MPI_SIZE_T, correspondents[i], 5, MPI_COMM_WORLD, &req[2 * i + 1]);
		}
		else
		{
			MPI_Isend(&tempsend[i][0], size, my_MPI_SIZE_T, correspondents[i], 6, MPI_COMM_WORLD, &req[2 * i]);
			MPI_Isend(&send_sizes[i][0], static_cast<int>(send_sizes[i].size()), my_MPI_SIZE_T, correspondents[i], 7, MPI_COMM_WORLD, &req[2 * i + 1]);
		}
	}
	vector<vector<size_t> > srecv(correspondents.size());
	...
	for (size_t i = 0; i < 2 * correspondents.size(); ++i)
	{
		...
		if (status.MPI_TAG % 2 == 0)
		{
			...
			if (location >= correspondents.size())
				throw UniversalError("Bad location in mpi exchange");
			irecv[location].resize(static_cast<size_t>(count));
			MPI_Recv(&irecv[location][0], count, my_MPI_SIZE_T, status.MPI_SOURCE, status.MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		else
		{
			MPI_Get_count(&status, my_MPI_SIZE_T, &count);
			...
			if (location >= correspondents.size())
				throw UniversalError("Bad location in mpi exchange");
			srecv[location].resize(static_cast<size_t>(count));
			MPI_Recv(&srecv[location][0], count, my_MPI_SIZE_T, status.MPI_SOURCE, status.MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	}
	for (size_t i = 0; i < correspondents.size(); ++i)
	{
